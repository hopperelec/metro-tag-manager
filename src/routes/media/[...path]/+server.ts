...
import { EXCLUDED_FILES, EXCLUDED_FOLDERS, MEDIAS_PATH, VALID_EXTENSIONS } from "$lib/constants";
...
export const GET: RequestHandler = async ({ params, request }) => {
  ...
  if (relativePath.includes("..")) error(403, "Forbidden");
  if (EXCLUDED_FOLDERS.some((folder) => relativePath.startsWith(folder)))
    error(403, "Forbidden");
  const filename = path.basename(relativePath);
  if (EXCLUDED_FILES.includes(filename)) error(403, "Forbidden");
  const ext = path.extname(filename).toLowerCase();
  if (!VALID_EXTENSIONS.includes(ext)) error(403, "Forbidden");

  const filePath = path.join(MEDIAS_PATH, relativePath);
  try {
  ...
  if (!stat.isFile()) error(403, "Forbidden");
