...
function validateTrain(train: unknown): boolean {
  return Array.isArray(train) &&
    train.length <= 10 &&
    train.every(validateTag);
}

function validatedBody(body: unknown) {
  if (!Array.isArray(body)) return null;
  if (!body.every((bodyItem: unknown) => {
    if (
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  await prisma.$transaction(
    body.map(({ id: mediaId, contextTags, trainTags }) => prisma.media.update({
      where: { id: mediaId },
      data: {
        contextTags: {
        ...
        trainTags: {
          deleteMany: { mediaId },
          create: trainTags.flatMap(
            (train, trainId) => train.map((tag) => ({ trainId, tag }))
          )
        }
      }
    }))
